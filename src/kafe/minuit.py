...
class Minuit:
    ...
    def set_parameter_values(self, parameter_names): #=None):
        '''Sets the fit parameters. If parameter_names=`None`, tries to infer defaults from the function_to_minimize.'''
        if len(parameter_names) == self.number_of_parameters:
            self.current_parameters = parameter_names
        else:
    ...
    def set_parameter_errors(self, param_errors=None):
        '''Sets the fit parameter errors. If parameter_names=`None`, sets the error to 1% of the parameter value.'''
    ...
    def get_parameter_name(self, param_nr):
        ...
        **param_nr** : int
        ...
        '''
        #if parameter_names is None: # infer parameters from FCN
