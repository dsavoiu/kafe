...
class Fit:
    ...
    *function_equation* : :math:`\LaTeX`-formatted string (optional)
    ...
    def __init__(self, dataset, fit_function, external_fcn=chi2):
        ...
        try:
            self.number_of_parameters = self.fit_function.number_of_parameters    #: the number of parameters
            self.current_parameter_values = self.fit_function.parameter_defaults      #: the current values of the parameters
            self.parameter_names = self.fit_function.parameter_names                  #: the names of the parameters
            self.latex_parameter_names = self.fit_function.latex_parameter_names  #: :math:`\LaTeX` parameter names

            self.function_equation = self.fit_function.get_function_equation('latex', 'full')
            self.function_label = self.fit_function.latex_name                    # get the function name in LaTeX
        except AttributeError:
            raise AttributeError("Fit-function object does not have the required properties. Did you maybe forget the `@FitFunction` decorator?")
        ...
        self.current_param_errors = [val/1000.0 for val in self.current_parameter_values]               #: the current uncertainties of the parameter
        ...
        self.minimizer = Minuit(self.number_of_parameters, self.call_external_fcn, self.parameter_names, self.current_parameter_values, None)   # initialize minimizer for fit
        self.minimizer.set_parameter_values(self.current_parameter_values)        # set Minuit's start parameters
    ...
    def call_external_fcn(self, *parameter_names):
        ...
        **parameter_names** : sequence of values
        ...
        return self.external_fcn(self.xdata, self.ydata, self.current_cov_mat, self.fit_function, parameter_names) 
    ...
    def do_fit(self, quiet=False, verbose=False):
        ...
        *verbose* : boolean (optional)
        ...
        if not quiet:
            ...
            print >>self.out_stream, "################"
            print >>self.out_stream, ''
            print >>self.out_stream, self.fit_function.get_function_equation('ascii', 'full')
            print >>self.out_stream, ''
    ...
    def project_x_covariance_matrix(self):
        ...
        proj_xcov_mat = np.asarray(self.dataset.get_cov_mat('x')) * \
            outer_product(self.fit_function.derive_by_x(self.dataset.get_data('x'), derivative_spacing, self.current_parameter_values)) 
