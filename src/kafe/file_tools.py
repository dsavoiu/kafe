...
#
           ...
           "*InitialParameters":'arr', # initial values and range of parameters
  ...
  for i in range(len(inputlines)):
    if words[i][0] in tokens:
      ...
      if tokens[curkey]=='':                  # expected input is string
        ...
        tokens[curkey]=[]
      elif(tokens[curkey])==None:   # expected input is text (python code)
        pass
    else:
      if(words[i][0][0]=='*'):
   ...
   # line without key is data, belonging to last key given
      if curkey=='*FitFunction':   # expect continuous text
        tokens[curkey].append(' '.join(words[i][:])) # join words to from line
      elif curkey=='*ConstrainedParameters':   # expect string + 2 floats
        tokens[curkey].append([words[i][0],float(words[i][1]),float(words[i][2])] )
  ...
  for key in setkeys:
    ...
    if type(tokens[key])==type([]):
      if     (key=='*xData'):
        ...
        if(cols >= 2):
          xerr=np.array([flist[i][1] for i in range(rows)],np.float32)
        else:
        ...
        if(cols >= 2):
          yerr=np.array([flist[i][1] for i in range(rows)],np.float32)
        else:
        ...
        rows=len(flist)
        cols=len(flist[0])
        parval=np.array([flist[i][0] for i in range(rows)],np.float32)
        if(cols >= 2):
          parerr=np.array([flist[i][1] for i in range(rows)],np.float32)
        else:
          parerr=None
        ...
        cparval=np.array([flist[i][1] for i in range(rows)],np.float32)
        cparerr=np.array([flist[i][2] for i in range(rows)],np.float32)
      else:
